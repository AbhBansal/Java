...
/*
 * InsertionSort class contains the logic for sorting according to Insertion sort algorithm
 */
	...
	public Student[] sort(Student[] stud) {
		for (int inCounter = 0; inCounter < stud.length; inCounter++) {
			for (int outCounter = inCounter; outCounter >0; outCounter--) {
				if(stud[outCounter].compareTo(stud[outCounter-1])==-1){
					swap(stud,outCounter,outCounter-1);
				}
				else
					break;
			}
